          ...
          <div className="p-8 space-y-4">
            {paymentOptions.map((option) => (
              <div
                ...
                className={`p-5 rounded-2xl border-2 cursor-pointer transition-all flex items-center justify-between ${selectedMethod === option.id
                    ? 'border-indigo-600 bg-indigo-50/30 dark:bg-indigo-600/10'
                    : 'border-slate-200 dark:border-white/10 hover:border-slate-300'
                  }`}
              >
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${selectedMethod === option.id ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-white/10 text-slate-400'
                    }`}>
                ...
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${selectedMethod === option.id ? 'border-indigo-600 bg-indigo-600' : 'border-slate-300'
                  }`}>
